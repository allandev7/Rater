...
public class Entrevista {
	...
	public void enviarFeedbackSemRelatorio(String emailEm, String conclusao, String comentario) {
		Properties props = new Properties();
        // Parâmetros de conexão com servidor Gmail 
        props.put("mail.smtp.host", "smtp.gmail.com");
        props.put("mail.smtp.socketFactory.port", "465");
        ...
        try {
            ...
            message.setRecipients(Message.RecipientType.TO, toUser);
            message.setSubject("Sobre a entrevista realizada. ");//Assunto
            String mensagem = "Gostamos muito de sua presença na entrevista da empresa "+Empresa.getNome()+
            		", você foi avaliado e foi " + conclusao +"\n \n";
            if(!comentario.equals("")) {
            	mensagem+= " Comentario do entrevistador: " + comentario;
            }
	...
	public ArrayList<dados> ENpesquisar2(String nome, int idEntrevistador) {
		...
		try {
			...
			pstmt.setInt(1, Empresa.getId());
			pstmt.setString(2, "%"+nome+"%");
			pstmt.setInt(3, idEntrevistador);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()) {
				dadosList.add(new dados(rs.getString(1), rs.getString(2),rs.getDate(3), rs.getInt(4),
						rs.getString(5), rs.getInt(6), rs.getString(7)));
